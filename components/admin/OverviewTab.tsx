...
import type { Bill, ElectricityRate, MeterReading, Profile } from '@/lib/utils'
...
export default function OverviewTab({ bills, rates, readings, users }: OverviewTabProps) {
    ...
    const currentRate = rates.find(r => r.month === CUR_M && r.year === CUR_Y)

    const tenantCards = tenants.map(t => ({
        ...t,
        bill: bills.find(b => b.user_id === t.id && b.month === CUR_M && b.year === CUR_Y),
        dues: bills.filter(b => b.user_id === t.id && b.status === 'pending').reduce((s, b) => s + b.amount, 0),
                    ...
                    <div className="card" key={t.id}>
                        <div className="row between mb3">
                            <div>
                                ...
                                <div className="small">{t.flat}</div>
                            </div>
                        ...
                        <div className="mt2 small muted">
                            Last reading: {t.lastReading
                                ? `${t.lastReading.reading_value} on ${new Date(t.lastReading.submitted_at).toLocaleDateString('en-IN')}`
                                : 'None submitted'}
                        </div>
                    </div>
                ))}
