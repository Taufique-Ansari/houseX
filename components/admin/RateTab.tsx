...
import type { ElectricityRate } from '@/lib/utils'
                ...
                <div className="card mt4">
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table className="tbl">
                            ...
                            <tbody>
                                {[...rates].sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()).map(r => (
                                    <tr key={r.id}>
                                        <td className="bold">{fmtM(r.month, r.year)}</td>
                                        <td className="mono">{r.total_units} kWh</td>
                                        <td className="mono">{fmtINR(r.total_amount)}</td>
