...
import type { MeterReading, Bill, ElectricityRate, Profile } from '@/lib/utils'
...
export default function ReadingsTab({ readings, bills, rates, users, token, onDataUpdated }: ReadingsTabProps) {
    ...
    const tenantReadings = readings
        .filter(r => r.user_id === tid)
        .sort((a, b) => new Date(b.submitted_at).getTime() - new Date(a.submitted_at).getTime())

    const curReading = readings.find(r => r.user_id === tid && r.month === selM && r.year === selY)
    const existingBill = bills.find(b => b.user_id === tid && b.month === selM && b.year === selY)
    ...
    const prevReading = readings.find(r => r.user_id === tid && r.month === prevMonth && r.year === prevYear)
                ...
                <div className="g3 mb4" style={{ gap: '0.75rem' }}>
                    <div className="fg" style={{ marginBottom: 0 }}>
                        ...
                        <select className="fi" value={tid} onChange={e => setTid(e.target.value)}>
                            {tenants.map(t => <option key={t.id} value={t.id}>{t.name} — {t.flat}</option>)}
                        </select>
                    </div>
                    <div className="fg" style={{ marginBottom: 0 }}>
